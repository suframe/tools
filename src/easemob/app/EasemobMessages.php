 ...
 * Time: 14:08
 */
namespace suframe\tools\easemob\app;
...
trait EasemobMessages
{
     ...
     * @throws EasemobError
     */
    public function sendMessageImg($users, $uuid, $share_secret, $file_name, $target_type = 'users',$width = 480, $height = 720, $send_user = 'admin')
    {
        if ( ! in_array($target_type, $this->target_array)) {
            throw new EasemobError('target_type 参数错误！');
        }
     ...
     * @throws EasemobError
     */
    public function sendMessageAudio($users, $uuid, $share_secret, $file_name, $target_type = 'users', $length = 10, $send_user = 'admin')
    {
        if ( ! in_array($target_type, $this->target_array)) {
            throw new EasemobError('target_type 参数错误！');
        }
     ...
     * @throws EasemobError
     */
    public function sendMessageVideo($users, $video_uuid, $video_share_secret, $video_file_name, $img_uuid, $img_share_secret, $target_type = 'users', $length = 10, $video_length = 58103, $send_user = 'admin')
    {
        if ( ! in_array($target_type, $this->target_array)) {
            throw new EasemobError('target_type 参数错误！');
        }
